...
volatile float referencePressure = 1030.13;//高度計算に使用する基準高度での大気圧[hPa]。ここは随時変える。
...
void GPSRUN() {
  ...
  if (accelermetor.z() > 0) {
    if (i <= THRESHOLD || (360 - THRESHOLD) <= i ) { //直進
      ...
      ledcWrite(CH2, 1000);
      delay(1000);
      if (Distance() < 10) { //ゴールから10mでゴール判定するように設定。ここで自律がとまる。
        while (1) {//GPSRUN停止。遠隔へ
          control();
...
void control() {
  ...
  if (client) {
    ...
    // stop autoボタンがクリックされた場合
    if (request.indexOf("api/v1/control/end") != -1) {
