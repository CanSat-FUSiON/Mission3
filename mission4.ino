...
volatile float referencePressure = 1048.85;//高度計算に使用する基準高度での大気圧[hPa]。ここは随時変える。
...
#define FREQ 18000   // PWM出力周波数（最大周波数 : 20kHz / 2の「bit数」乗）
  ...
  if (float(altitude()) > float(gnd_T) + 20) {
    ...
    while (1) {
      ...
      if (float(altitude()) < 1) {
        ...
        //キャリブレーションをここで行う
        Serial.print("1");
        ledcWrite(CH1, 1500); //フル正回転
        ledcWrite(CH2, 1500); //フル正回
        delay(5000);
        Serial.print("2");
...
void remort() {
  ...
  if (client) {
    ...
    // rightbackボタンがクリックされた場合
    if (request.indexOf("api/v1/control/left_back") != -1) {
      ledcWrite(CH1, 2596);
      ledcWrite(CH2, 2048);
      delay(1000);
      ledcWrite(CH1, 2048); //右タイヤ正回転
    ...
    // backボタンがクリックされた場合
    if (request.indexOf("api/v1/control/right_back") != -1) {
      ledcWrite(CH1, 4096);
      ledcWrite(CH2, 2596);
      delay(1000);
      ledcWrite(CH1, 2048); //右タイヤ正回転
