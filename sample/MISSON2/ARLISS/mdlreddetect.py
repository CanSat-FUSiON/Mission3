...
# def main(image, f_mm, diagonal_mm):

#     image_output(red_masks_get(image), 'sample_red.jpg')
#     image_output(gray_get(image), 'sample_gray.jpg')
#     image_output(binary_get(image), 'sample_binary.jpg')
#     center_output(image, 'sample_center.jpg', center_get(image))

#     print('Size:', size_get(image))
#     print('Occupancy:', occ_get(image)*100, '%')
#     print('Center:', center_get(image))
#     print('Rotation:', rot_get(center_get(image),
#           size_get(image), f_mm, diagonal_mm))
...
# # 重心座標と抽出画像を出力する関数
# def center_output(image, image_name, center):
#     img_arrange = cv2.cvtColor(red_masks_get(
#         image), cv2.COLOR_BGR2RGB)  # グラフにするために形式変換
...
def red_masks_get(image):
    ...
    # HSVでの色抽出
    hsvLower_0 = np.array([0, 100, 120])    # 抽出する色の下限0(赤の抽出のため二つにわけて合成が必要)
    hsvLower_1 = np.array([170, 100, 120])  # 抽出する色の下限1(赤の抽出のため二つにわけて合成が必要)
...
# main(image, 4.8, 4.5)  # ESP32-CAM用
# main(image, 4, 7.14)  # iPhone11用
