...
const IPAddress ip(172, 20, 10, 9);
const IPAddress gateway(192,168, 3, 1);
const IPAddress subnet(255, 255, 255, 0);
...
void handleGPS1() {
  ...
  delay(3000);
  run_cmd(1,2000);
  server.send(200, "text/plain", "run GPS");
  Serial.println("run GPS");  
}
...
void setup(void) {
 ...
 if (!WiFi.config(ip,ip,subnet)){
  ...
  server.on("/GPS1", handleGPS1);
  server.on("/GPS2", handleGPS2);
  server.on("/GPS3", handleGPS3);
