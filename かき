...
float LatA = 30.438882828;
float LongA = 130.927093506;      //コーンの緯度・経度入力
  ...
  if (accelermetor.z() > 0) {
    if (i <= THRESHOLD || (360 - THRESHOLD) <= i ) { //直進
      ...
      if (i <= 180) {
        ...
        if (i >= 270) {
          Serial.println("よわめにまわれみぎ");
          ledcWrite(CH1, 2000); //右タイヤブレーキ
          ledcWrite(CH2, 1500); //左タイヤ正回転
          delay(500);
        } else {
          Serial.println("つよめにまわれみぎ");
          ledcWrite(CH1, 2000); //右タイヤブレーキ
